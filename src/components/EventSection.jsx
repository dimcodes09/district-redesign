...
const EventSection = () => {
  ...
  const applyFilters = (event) => {
    if (searchTerm && !event.title.toLowerCase().includes(searchTerm)) return false;
    if (filters.genre && event.category?.toLowerCase() !== filters.genre) return false;
    if (filters.location && event.location?.toLowerCase() !== filters.location)
      return false;

    if (filters.price) {
      const [min, max] = filters.price.split("-").map(Number);
      const value = parseInt(event.price.replace(/[₹^\s]/g, ""));
  ...
  return (
    <section
      id="events"
      className="px-6 sm:px-10 py-16 bg-gradient-to-b from-white to-purple-50 transition-all duration-300"
    >
      ...
      <div className="text-center mb-10 mt-6">
        ...
        <p className="text-gray-600 mt-2 text-sm">
          Find concerts, stand-ups, and festivals near you
        </p>
      </div>
